...
# --- paths ---
data_dir = Path("data")
in_path  = data_dir / "01_raw.parquet"
out_path = data_dir / "02_standardised.parquet"
...
element_mapping = {
    ...
    # gold — fire assay preferred over ICP for Au (lower detection limit)
    "Au_FA_PPB":  "gold_ppb",
    # pathfinders
...
# gold fire assay is in PPB — convert to PPM for consistency
# all other elements already in PPM
if "gold_ppb" in gdf.columns:
    gdf["gold_ppm"] = gdf["gold_ppb"] * 0.001
    gdf = gdf.drop(columns=["gold_ppb"])
    logger.info("gold converted ppb → ppm")
...
gdf["_method_choice"]    = "ICP primary; fire assay for Au"
